...
export default function SocialCallout() {
  ...
  const socialChannels = isEn ? [
    {
      name: 'Official Telegram Channel & Advisory',
      desc: 'Fast communication, latest technology updates, and technical IT advisory',
      icon: (
      ...
      btnText: 'Start Chat on Telegram'
    },
    {
      name: 'WhatsApp Support & Consulting',
      desc: 'Meeting coordination, project inquiries, and direct executive support',
      icon: (
      ...
      btnText: 'Send Message on WhatsApp'
    },
    {
      name: 'Professional LinkedIn Network',
      desc: 'Review 20 years of background, professional networking, and executive IT articles',
      icon: (
    ...
    },
    {
      ...
      desc: 'Explore open-source codebases, software architecture, and development tools',
      icon: (
    ...
    },
    {
      ...
      desc: 'بررسی سوابق ۲۰ ساله، شبکه‌سازی حرفه‌ای و مقالات IT',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-sky-400">
          <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/>
        </svg>
  ...
  return (
    <section className="py-20 px-4 relative overflow-hidden bg-gradient-to-b from-black via-[#0d0f17] to-black border-y border-white/10 font-vazir" dir={isEn ? 'ltr' : 'rtl'}>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/30 text-orange-400 text-xs font-bold mb-4">
            <Sparkles size={14} /> {isEn ? 'Direct & Fast Communication Bridges' : 'پل‌های ارتباطی مستقیم و سریع'}
          </div>
          <h2 className="text-3xl md:text-5xl font-black mb-4 tracking-tight">
            <span className="bg-gradient-to-r from-orange-400 via-amber-300 to-blue-400 bg-clip-text text-transparent">
              {isEn ? 'Let’s Stay in Touch' : 'با ما در تماس باشید'}
            </span>
          </h2>
          <p className="text-zinc-300 text-base md:text-lg font-light leading-relaxed">
            {isEn ? 'For specialized enterprise network consulting, AI automation architecture, or quick technical inquiries, connect directly across these official channels.' : 'برای مشاوره تخصصی شبکه و هوش مصنوعی، طراحی سیستم‌های سازمانی یا دریافت پاسخ سوالات فنی، از طریق کانال‌های زیر به صورت مستقیم در ارتباط باشید.'}
          </p>
        </div>
          ...
          {socialChannels.map((item, idx) => (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-4 border-t border-white/10 text-sm font-bold text-white/90 group-hover:text-amber-400 transition-colors">
                ...
                <span className={`transform ${isEn ? 'group-hover:translate-x-2' : 'group-hover:-translate-x-2'} transition-transform text-lg`}>
                  {isEn ? '→' : '←'}
                </span>
              </div>
            </a>
        ...
        <div className="glass rounded-3xl p-8 border border-white/15 bg-gradient-to-r from-orange-500/10 via-black to-blue-500/10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
          <div className={`flex items-center gap-5 text-center ${isEn ? 'md:text-left' : 'md:text-right'}`}>
            ...
            <div>
              <h4 className="text-xl font-bold text-white mb-1">
                {isEn ? 'Need Immediate Phone or Technical Consultation?' : 'نیاز به مشاوره فوری یا تماس تلفنی دارید؟'}
              </h4>
              <p className="text-zinc-400 text-sm">
                {isEn ? 'Send official inquiries to ' : 'ارسال ایمیل به '}
                <span className="text-amber-400 font-mono">info@ehsansalehi.ir</span>
                {isEn ? ' or connect during business hours.' : ' یا تماس در ساعات اداری'}
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-4 shrink-0">
            ...
            >
              <Mail className="w-4 h-4 text-amber-400" /> {isEn ? 'Send Official Email' : 'ارسال ایمیل رسمی'}
            </a>
            <a
            ...
            >
              {isEn ? 'Instant WhatsApp Support 🚀' : 'پشتیبانی فوری واتساپ 🚀'}
            </a>
          </div>
        </div>
      </div>
