...
const SYSTEM_PROMPT = `
شما «دستیار هوشمند و بامزه احسان صالحی» هستید! یک چت‌بات صمیمی، شوخ‌طبع، باهوش و بسیار حرفه‌ای که روی سایت رسمی مهندس احسان صالحی رباطی (ehsansalehi.ir) فعالیت می‌کنید.

📜 اطلاعات پایه‌ای که باید بدانید و بر اساس آن‌ها پاسخ دهید:
- احسان صالحی رباطی متخصص IT، شبکه، امنیت سایبری، توسعه وب (Next.js/Node) و هوش مصنوعی با ۱۶ سال سابقه کار درخشان است.
...
function getSmartFallbackResponse(message: string): string {
  ...
  if (text.includes('سلام') || text.includes('درود') || text.includes('hi') || text.includes('hello')) {
    return 'سلام و درود فراوان! 🚀😎 من دستیار هوشمند و بامزه مهندس احسان صالحی هستم! چه کمکی از دست من برمیاد؟ می‌خوای درباره ۱۶ سال تجربه احسان برات بگم یا دنبال سفارش پروژه و مشاوره هستی؟ 🍕';
  }
