...
export default async function NewsPage({ params }: NewsPageProps) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#0a0a0a] to-[#1a1a2e] text-white py-12 px-4">
      <article className="max-w-4xl mx-auto" dir="rtl">
        ...
        {news.source_url && news.source_url !== news.original_url && (
          <div className="mt-10 pt-6 border-t border-white/10 text-center">
            <a href={news.source_url} target="_blank" rel="noopener noreferrer" className="text-zinc-500 hover:text-amber-400 transition-colors text-sm">
        ...
        <div className="mt-10 pt-6 border-t border-white/10 flex items-center gap-4 flex-wrap">
          ...
          <button className="p-2 bg-blue-500/20 hover:bg-blue-500/40 rounded-full transition-colors" onClick={() => navigator.share?.({ title: news.title, url: window.location.href })}>
            <svg className="w-5 h-5 text-blue-400" fill="currentColor" viewBox="0 0 24 24">
              <path d="M18.77 7.46H14.5v-1.9c0-.9.6-1.1 1-1.1h3V.5h-4.33C10.24.5 9.5 3.44 9.5 5.32v2.15h-3v4h3v12h5v-12h3.85l.42-4z" />
            </svg>
          </button>
          <button className="p-2 bg-sky-500/20 hover:bg-sky-500/40 rounded-full transition-colors" onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(news.title)}&url=${encodeURIComponent(window.location.href)}`, '_blank')}>
          ...
          <button className="p-2 bg-green-500/20 hover:bg-green-500/40 rounded-full transition-colors" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(news.title + ' ' + window.location.href)}`, '_blank')}>
        ...
        <div className="mt-12 pt-6 border-t border-white/10">
          <p className="text-sm text-zinc-400 text-center">
            ✨ این خبر را با دیگران به اشتراک بگذارید
          </p>
        </div>
      </article>
    </main>
